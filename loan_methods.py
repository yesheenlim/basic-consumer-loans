...
def get_cashflow_schedule(apr,principal,nterms,date0,dateT):
    ...
    df['Date'] = get_cashflow_dates(date0,dateT,nterms)
    
    return df

def get_cashflow_dates(date0,dateT,nterms):
    """Returns list of strings with payment dates.

    Args:
        date0: Start date.
        dateT: Repayment date.
        nterms: Number of months.

    Returns:
        list: List of strings of dates.
    """
    date_list = [date0]
    for n in xrange(int(nterms)):
